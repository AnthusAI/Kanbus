...
def test_main_delegates_to_run_daemon(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    called: dict[str, Path] = {}
    monkeypatch.setattr(daemon, "run_daemon", lambda root: called.setdefault("root", root))
    daemon.main(["--root", str(tmp_path)])
    assert called["root"] == tmp_path


def test_module_main_invocation_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    called: dict[str, Path] = {}
    monkeypatch.setattr(
