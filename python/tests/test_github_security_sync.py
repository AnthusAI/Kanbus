...
def test_parse_next_link() -> None:
    ...
    assert (
        _parse_next_link('<https://api.github.com/foo?page=2>; rel="next"')
        == "https://api.github.com/foo?page=2"
    )
...
def test_validate_dependabot_state_rejects_invalid_value():
    ...
    with pytest.raises(
        GithubSecuritySyncError, match="invalid dependabot state 'invalid'"
    ):
...
def test_map_dependabot_to_kanbus_sets_expected_fields():
    ...
    issue_no_pkg = _map_dependabot_to_kanbus(
        alert_no_package, "AnthusAI/Kanbus", "kbs-epic.1"
    )
...
def test_append_and_find_marker_round_trip():
    ...
    assert (
        _extract_marker(described, "kanbus-gh-alert:dependabot|") == "AnthusAI/Kanbus|7"
    )
    ...
    issue = IssueData.model_validate(
        {
            ...
            "updated_at": now,
            "closed_at": None,
            "custom": {"github_provider": "dependabot"},
        }
    )
...
def test_build_beads_indexes_read_markers():
    now = datetime.now(timezone.utc).isoformat()
    alert_desc = _append_marker(
        "alert description", _metadata_marker_alert("example/repo", 11)
    )
    task_desc = _append_marker(
        "task description", _metadata_marker_target("example/repo", "Cargo.lock")
    )
...
def test_find_existing_security_initiative_prefers_latest_github_labeled_issue(
    tmp_path,
):
    issues_dir = tmp_path / "issues"
    issues_dir.mkdir()
...
def test_find_existing_dependabot_epic_filters_by_parent_and_label(tmp_path):
    ...
    ids = {
        matching.identifier,
        wrong_parent.identifier,
        newer_match.identifier,
        "missing",
    }
...
def test_resolve_dependabot_epic_prefers_existing_and_configured_issue(tmp_path):
    ...
    resolved_existing = _resolve_dependabot_epic(
        issues_dir, "kbs", None, False, existing
    )
    assert resolved_existing == epic.identifier

    resolved_configured = _resolve_dependabot_epic(
        issues_dir, "kbs", "kbs-epic.9", False, existing
    )
...
def test_resolve_beads_task_finds_existing_with_label(tmp_path):
    ...
    with mock.patch("kanbus.github_security_sync.update_beads_issue") as mock_update:
        assert (
            _resolve_beads_task(
                tmp_path, "test-repo", "123", "kanbus-2", 2, False, index
            )
            == "kanbus-5"
        )
    ...
    assert (
        _resolve_beads_task(tmp_path, "test-repo", "123", "kanbus-2", 2, True, index)
        == "kanbus-5"
    )
...
def test_pull_dependabot_from_github_success(mock_fetch, mock_load_project, tmp_path):
    ...
    config = GithubSecurityConfiguration(
        repo="example/repo", dependabot=DependabotConfiguration(state="open")
    )
    ...
    with pytest.raises(
        GithubSecuritySyncError, match="issues directory does not exist"
    ):
        pull_dependabot_from_github(tmp_path, config, "PRJ", False)
...
@mock.patch("kanbus.github_security_sync._fetch_dependabot_alerts")
@mock.patch.dict(os.environ, {"GITHUB_TOKEN": "fake_token"}, clear=True)
def test_pull_dependabot_from_github_beads_success(
    mock_fetch, mock_update, mock_create, mock_load, tmp_path
):
    ...
    now = datetime.now(timezone.utc).isoformat()
    alert_desc = _append_marker(
        "alert description", _metadata_marker_alert("example/repo", 42)
    )
    task_desc = _append_marker(
        "task description", _metadata_marker_target("example/repo", "Cargo.toml")
    )
...
def test_resolve_beads_epic_with_configured_id(tmp_path):
    ...
    assert (
        _resolve_beads_epic(tmp_path, issues, "kanbus-configured", "kanbus-1", False)
        == "kanbus-configured"
    )
...
@mock.patch("kanbus.project.load_project_directory")
@mock.patch.dict(os.environ, {"GITHUB_TOKEN": "fake_token"}, clear=True)
def test_pull_dependabot_from_github_exception(
    mock_load_project, mock_read, mock_fetch, tmp_path
):
    ...
    with mock.patch.object(Path, "exists", return_value=True):
        ...
        with mock.patch(
            "kanbus.github_security_sync._build_alert_index",
            return_value={"example/repo#42": "kbs-1"},
        ):
...
def test_missing_coverage_edges(tmp_path):
    ...
    write_issue_to_file(
        wrong_type_init, _issue_path(issues_dir, wrong_type_init.identifier)
    )
    write_issue_to_file(
        wrong_type_epic, _issue_path(issues_dir, wrong_type_epic.identifier)
    )
    write_issue_to_file(
        wrong_title_epic, _issue_path(issues_dir, wrong_title_epic.identifier)
    )

    _find_existing_security_initiative(issues_dir, {wrong_type_init.identifier})
    _find_existing_dependabot_epic(
        issues_dir,
        {wrong_type_epic.identifier, wrong_title_epic.identifier},
        "kbs-init",
    )
