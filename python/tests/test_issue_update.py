...
def test_update_issue_no_updates_requested(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
...
def test_update_issue_title_duplicate_and_normalization(
    ...
    with pytest.raises(issue_update.IssueUpdateError, match="duplicate title"):
        issue_update.update_issue(
            tmp_path, "kanbus-1", "  New  ", None, None, None, False
        )


def test_update_issue_parent_resolution_and_validation_paths(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    with pytest.raises(issue_update.IssueUpdateError, match="bad parent"):
        issue_update.update_issue(
            tmp_path, "kanbus-1", "New", None, None, None, False, parent="p"
        )
    ...
    monkeypatch.setattr(
        issue_update.Path,
        "glob",
        lambda self, pattern: [child_path] if self == issues_dir else [],
    )
    monkeypatch.setattr(
        issue_update,
        "read_issue_from_file",
        lambda _p: build_issue("child", issue_type="bug", parent="kanbus-1"),
    )
    monkeypatch.setattr(
        issue_update,
        "validate_parent_child_relationship",
    ...
    with pytest.raises(issue_update.IssueUpdateError, match="bad child"):
        issue_update.update_issue(
            tmp_path, "kanbus-1", "New", None, None, None, False, issue_type="story"
        )


def test_update_issue_type_status_validation_errors(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _setup(monkeypatch, tmp_path)
    with pytest.raises(issue_update.IssueUpdateError, match="unknown issue type"):
        issue_update.update_issue(
            tmp_path, "kanbus-1", "New", None, None, None, False, issue_type="nope"
        )
    ...
    with pytest.raises(issue_update.IssueUpdateError, match="bad status"):
        issue_update.update_issue(
            tmp_path, "kanbus-1", "New", None, "in_progress", None, False
        )


def test_update_issue_happy_path_with_labels_and_claim(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    issue = build_issue(
...
def test_update_issue_same_type_becomes_none(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    issue = build_issue("kanbus-1", title="Old", status="open", issue_type="task")
...
def test_update_issue_event_write_failure_rolls_back(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    _setup(monkeypatch, tmp_path)
    writes: list[str] = []
...
def test_find_duplicate_title_helper(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(issue_update, "read_issue_from_file", fake_read)
    assert (
        issue_update._find_duplicate_title(issues_dir, "same", "kanbus-current")
        == "kanbus-1"
    )
