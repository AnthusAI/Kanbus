...
from kanbus.models import CategoryDefinition, DependencyLink, IssueComment, StatusDefinition

from test_helpers import build_issue, build_project_configuration


def _comment(text: str, author: str = "dev", comment_id: str | None = None) -> IssueComment:
...
def test_issue_display_color_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(issue_display.click, "get_current_context", lambda silent=True: SimpleNamespace(color=True))
    assert issue_display._should_use_color() is True

    monkeypatch.setattr(issue_display.click, "get_current_context", lambda silent=True: None)
    ...
    monkeypatch.setattr(issue_display.click, "style", lambda text, fg=None: f"[{fg}]{text}")
...
def test_render_description_and_comments_success(monkeypatch: pytest.MonkeyPatch) -> None:
    issue = build_issue("kanbus-1", title="T", status="open")
    issue.description = "{{ desc }}"
    issue.comments = [_comment("{{ c1 }}"), _comment("{{ c2 }}")]
    monkeypatch.setattr(wiki, "render_template_string", lambda value, _issues: f"R:{value}")

    description, comments = issue_display._render_description_and_comments(issue, [issue])
...
def test_render_description_and_comments_empty_description(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_format_issue_for_display_sections_and_context(monkeypatch: pytest.MonkeyPatch) -> None:
    configuration = build_project_configuration()
    configuration.type_colors["task"] = "green"
    configuration.categories = [CategoryDefinition.model_validate({"name": "To do", "color": "magenta"})]
    configuration.statuses = [
        StatusDefinition.model_validate({"key": "open", "name": "Open", "category": "To do"})
    ]
    configuration.priorities[2].color = "bright_blue"

    issue = build_issue("kanbus-1", title="My Title", status="open", issue_type="task")
    issue.description = "desc"
    issue.dependencies = [DependencyLink.model_validate({"target": "kanbus-2", "type": "blocked-by"})]
    issue.comments = [
        _comment("first", comment_id="abcdef123456"),
        SimpleNamespace(id=None, author="", text="second"),
    ]

    monkeypatch.setattr(issue_display, "format_issue_key", lambda identifier, _ctx: f"FMT:{identifier}")
    monkeypatch.setattr(issue_display, "_render_description_and_comments", lambda _issue, _all: ("rendered desc", ["rc1", "rc2"]))
...
def test_format_issue_for_display_with_colorized_muted_fields(
    ...
    issue.assignee = None
    monkeypatch.setattr(issue_display, "format_issue_key", lambda identifier, _ctx: identifier)
    monkeypatch.setattr(issue_display.click, "style", lambda text, fg=None: f"[{fg}]{text}")
...
def test_format_issue_for_display_uses_explicit_status_color() -> None:
    configuration = build_project_configuration()
    configuration.categories = [CategoryDefinition.model_validate({"name": "To do", "color": "blue"})]
    configuration.statuses = [
    ...
    rendered = issue_display.format_issue_for_display(issue, configuration=configuration, use_color=False)
    assert "Status: open" in rendered


def test_issue_line_color_resolution_helpers() -> None:
    configuration = build_project_configuration()
    configuration.categories = [CategoryDefinition.model_validate({"name": "To do", "color": "red"})]
    configuration.statuses = [
        StatusDefinition.model_validate({"key": "open", "name": "Open", "category": "To do"})
    ]
...
def test_issue_line_safe_color_and_widths(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert issue_line._safe_color(lambda t, fg=None: f"[{fg}]{t}", "x", "red") == "[red]x"

    one = build_issue("kanbus-1", issue_type="task", status="open")
    two = build_issue("kanbus-22", issue_type="bug", status="in_progress", parent="kanbus-1")
    widths = issue_line.compute_widths([one, two])
    assert widths["identifier"] >= len("kanbus-22")
    assert widths["status"] >= len("in_progress")

    monkeypatch.setattr(issue_line, "format_issue_key", lambda identifier, project_context=False: identifier.replace("kanbus-", "k-") if project_context else identifier)
...
def test_issue_line_format_non_porcelain_with_color_paths(
    ...
    configuration.categories = [CategoryDefinition.model_validate({"name": "To do", "color": "cyan"})]
    configuration.statuses = [
        StatusDefinition.model_validate({"key": "open", "name": "Open", "category": "To do"})
    ]
    ...
    monkeypatch.delenv("NO_COLOR", raising=False)
    monkeypatch.setattr(issue_line, "format_issue_key", lambda identifier, project_context=False: identifier)
