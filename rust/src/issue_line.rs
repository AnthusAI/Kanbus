...
mod tests {
    ...
    #[test]
    fn computes_widths_with_parent() {
        let mut issue = sample_issue("kanbus-abc1234");
        issue.parent = Some("kanbus-parent".to_string());
        let widths = compute_widths(&[issue], false);
        assert!(widths.identifier > 0);
        assert!(widths.parent > 0);
        assert!(widths.status > 0);
        assert!(widths.priority > 0);
    }

    #[test]
    fn formats_porcelain_line() {
    ...
    #[test]
    fn formats_non_porcelain_line() {
        let issue = sample_issue("kanbus-abc1234");
        let line = format_issue_line(&issue, None, false, false, None, Some(false));
        let expected_id = format_issue_key(&issue.identifier, false);
        assert!(line.contains(&expected_id));
    ...
    #[test]
    fn falls_back_when_color_unknown() {
        assert_eq!(parse_color("nope"), None);
        assert_eq!(status_color("unknown", None), Some(AnsiColors::White));
        assert_eq!(priority_color(99, None), Some(AnsiColors::White));
        assert_eq!(type_color("unknown", None), Some(AnsiColors::White));
    }
}
