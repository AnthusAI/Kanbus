...
mod tests {
    ...
    #[test]
    fn compare_semver_cores_handles_git_describe_suffixes() {
        let cases = [
    ...
    #[test]
    fn read_required_kanbus_version_rejects_invalid_contents() {
        let temp = tempdir().expect("tempdir");
        fs::write(temp.path().join("kanbus-version"), "not-a-version").expect("write");
        let error = read_required_kanbus_version(temp.path())
            .expect_err("expected invalid file error");
        assert_eq!(error.message(), INVALID_KANBUS_VERSION_MESSAGE);
    }

    #[test]
    fn enforce_kanbus_version_reports_unparseable_running_version() {
        let temp = tempdir().expect("tempdir");
        fs::write(temp.path().join("kanbus-version"), "1.0.0\n").expect("write");
        let error = enforce_kanbus_version(temp.path(), "release-candidate")
            .expect_err("expected running version error");
        assert_eq!(
            error.message(),
